/*
 * Filename: HashTable.java Project: Final Project Date: Apr 24, 2019 Authors: Lu Duan
 *
 * Semester: Spring 2019 Course: CS400 Instructor: Deppeler (deppeler@cs.wisc.edu)
 *
 */
 ...
 * 
 * Hashing algorithm: key.hashCode % tableSize to get the hash index
 * 
 * When insert, check if head node is null, if yes, insert to table, otherwise append to the end of
 * the bucket When get and remove, check if the spot has been taken, if yes, get or remove from the
 * bucket, otherwise remove from table
 * 
...
public class HashTable<K extends Comparable<K>, V> implements HashTableADT<K, V> {
  ...
  /*
   * table: array of hashNode<K, V> numKeys: number of <key, value> pair in table tableSize: the
   * size of table. When loadFactor reach its threshold, tableSize * 2 + 1 loadFactorThres:
   * threshold for doubling tableSize
   */
  ...
  private void put(hashNode<K, V>[] ht, K key, V value) throws DuplicateKeyException {
    ...
    hashNode<K, V> newNode = new hashNode<K, V>(key, value); // Construct the new hashNode for
                                                             // insert
  ...
  /*
   * If key is found, remove the key,value pair from the table decrease number of keys. return true
  ...
  private boolean delete(K key) throws IllegalNullKeyException {
    ...
    if (node == null)
      ...
      hashNode<K, V> nextNode = node.next; // to trace the previous node in case current node need
                                           // to be deleted
